...
import codeOrchestra.colt.as.model.RunTargetType
...
public class TargetFormController implements Initializable {
    ...
    private RunTargetModel model = ModelStorage.getInstance().project.runTargetModel
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        target.selectedToggleProperty().addListener({ observableValue, Toggle old_toggle, Toggle new_toggle ->
            model.target = RunTargetType.values()[target.toggles.indexOf(new_toggle)]
        ...
        (model.target() as StringProperty).addListener({ prop, oldVal, newVal ->
            if (newVal) {
                controls*.disable = true
                RunTargetType targetType = RunTargetType.valueOf("" + newVal)
