...
class TestList extends Application {
    ...
    private javafx.collections.ObservableList<LogMessage> createTestLogList() {
        FXCollections.observableArrayList(
                new LogMessage("com.codeOrchestra.*:8", Level.DEBUG, """
ListView Selection / Focus APIs To track selection and focus, it is necessary to become familiar with the SelectionModel and
...
""", 10, ""),
                new LogMessage("com.codeOrchestra.*:8", Level.ALL, """
...
""", 10, ""),
                new LogMessage("com.codeOrchestra.*:18", Level.FATAL, """
A simple example of how to create and populate a ListView of names (Strings) is shown here:
""", 10, ""),
                new LogMessage("com.codeOrchestra.*:8", Level.ERROR, """
...
""", 10, ""),
                new LogMessage("com.codeOrchestra.*:28", Level.INFO, """
The elements of the ListView are contained within the items ObservableList. This ObservableList is automatically observed by the ListView, such that any changes that occur inside the ObservableList will be automatically shown in the ListView itself. If passying the ObservableList in to the ListView constructor is not feasible, the recommended approach for setting the items is to simply call:
""", 10, ""),
                new LogMessage("com.codeOrchestra.*:33", Level.TRACE, """The issue with the approach shown above is that the content list is being copied into the items list - meaning that subsequent changes to the content list are not observed, and will not be reflected visually within the ListView.
""", 10, ""),
